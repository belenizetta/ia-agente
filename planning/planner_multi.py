...
class MultiServicePlanner:
    ...
    def find_relevant_files(
        ...
        # Pre-filtrar por lenguaje dominante para no mezclar archivos Java con TS o viceversa
        is_java = "java" in [l.lower() for l in svc_langs]
        is_ts = any(l.lower() in ("typescript", "javascript") for l in svc_langs)
        java_exts = {".java", ".xml", ".properties", ".yaml", ".yml", ".gradle", ".sql"}
        ts_exts   = {".ts", ".html", ".scss", ".css", ".json"}

        def lang_filter(path: str) -> bool:
            ext = os.path.splitext(path)[1].lower()
            if is_java and not is_ts:
                return ext in java_exts
            if is_ts and not is_java:
                return ext in ts_exts
            return True  # proyecto mixto o desconocido → sin filtro

        files = [f for f in files if lang_filter(f)]
        ...
        # 2) Búsqueda directa por nombre de clase/entidad mencionada en el prompt
        #    Esto garantiza que si el prompt dice "EventListener" se encuentre EventListener.java
        hints = set(
        ...
        for f in files:
            ...
            # Match exacto por nombre de clase (case-insensitive)
            f_basename = os.path.splitext(os.path.basename(f))[0].lower()
            if any(h.lower() == f_basename or h.lower() in f_lower for h in hints):
                add_if(f)
    ...
    def plan(
        ...
        all_known_services = list(project_info.get("services", {}).keys())

        # 2) El Router semántico es autoritativo cuando devuelve un subconjunto real.
        #    Solo se recurre a la heurística si el router falló o devolvió todo.
        router_is_authoritative = (
            involved_services
            and isinstance(involved_services, list)
            and len(involved_services) < len(all_known_services)
        )

        if router_is_authoritative:
            # El router ya filtró: confiamos en él sin mezclar heurísticas.
            involved = [s for s in involved_services if s in all_known_services]
        else:
            # El router devolvió todos (o falló): usamos detección heurística propia.
            heuristic_services = self.detect_involved_services(prompt, project_info, intent_info)
            involved = heuristic_services or all_known_services

        if not involved:
            involved = all_known_services

        # 3) Aplicar filtro de seguridad (Primary Owner) solo cuando no fue el router quien decidió.
        if not router_is_authoritative:
            involved = self._apply_primary_owner_filter(involved, prompt, intent_info, project_info)

        tasks: List[Dict[str, Any]] = []

        # 4) Un único task por servicio usando SOLO el intent primario
        primary = intent_info.get("primary_intent", intents[0])
        action_norm = primary if primary in self.basic_actions else "modify_code"

        for svc in involved:
            # Limitar archivos para controlar el consumo de tokens
            max_files = int(os.getenv("MAX_FILES_PER_TASK", "5"))
            files = self.find_relevant_files(svc, project_info, intent_info, prompt, limit=max_files)
